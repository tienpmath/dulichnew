...
export default function Product({product}: {
	...
	const productCat = product.categories.length > 0 ? product.categories[0] : undefined
	if(engVer){
		product.title = (product.langEn as any)?.title || product.title
		product.description = (product.langEn as any)?.description || product.description
		product.body = (product.langEn as any)?.body || product.body
		product.extraBody = (product.langEn as any)?.extraBody || product.extraBody
		product.extraBody1 = (product.langEn as any)?.extraBody1 || product.extraBody1
		product.extraBody2 = (product.langEn as any)?.extraBody2 || product.extraBody2

		if(productCat){
			productCat.name = (productCat.langEn as any)?.name || productCat.name
			if(productCat?.parentCategory){
				productCat.parentCategory.name = (productCat?.parentCategory?.langEn as any)?.name || productCat?.parentCategory?.name
			}
		}
	}

	const catSlug = `${productCat?.parentCategory ? productCat?.parentCategory?.slug : productCat?.slug}`
	...
	return (
		<>
					...
					data={[
						...
						Boolean(productCat)
							? {
								...
								href: `${productCat?.parentCategory ? '/'+productCat?.parentCategory?.slug : ''}/${productCat?.slug}`
							} : {},
						{
							title: product.title
						}
			...
			<div className={'product-detail container'}>
					...
					<div className={'flex-grow mb-8'}>
						<div className={'w-full max-w-xl lg:px-5'}>
							<div className={'grid grid-cols-1 gap-3 lg:gap-5'}>
								<h1 className={'text-2xl md:text-4xl font-bold leading-snug md:leading-snug'}>
									{product.title}
								</h1>
								...
								{product.extraBody && (
									<DisplayProse content={String(product.extraBody)} className={'prose-p:my-2 prose-img:hidden'}/>
								)}

								<div className={'my-2'}>
									<Button
										asChild
									  className={'motion-translate-y-loop-[15px] uppercase bg-red-600 hover:bg-red-700 text-white px-8 py-3 h-auto text-base font-medium flex w-fit shadow-lg transition-all duration-300'}
									  variant={'primary'}
									>
										<Link href={'https://zalo.me/0979788685'}>
											<PhoneCallIcon className={'motion-rotate-loop-[30deg] motion-duration-300 size-5 mr-2'}/> {t("menu.contact")}
										</Link>
									</Button>
								</div>
									...
									{productCat && (
										<p>
											...
											{Boolean(productCat?.parentCategory) && (
												<Link className={'text-indigo-600 hover:underline'} href={`${productCat.parentCategory?.slug}`}>{productCat.parentCategory?.name}, </Link>
											)}
											<Link className={'text-indigo-600 hover:underline'} href={`${productCat?.parentCategory ? '/'+productCat?.parentCategory?.slug : ''}/${productCat?.slug}`}>{productCat.name}</Link>
										</p>
									)}
									<ShareProductButtons href={`${process.env.NEXT_PUBLIC_APP_URL}/san-pham/${product.slug}`}/>
								</div>
				...
				<Tabs defaultValue="description" className="mt-5 lg:mt-10 rounded-md bg-white p-5 lg:p-8">
					<TabsList className="font-bold uppercase xl:text-lg bg-transparent mx-auto flex flex-wrap h-auto">
						<TabsTrigger className={'data-[state=active]:text-red-600 data-[state=active]:border-b-red-500 rounded-none bg-transparent uppercase border-b-2 p-4 py-3'} value="description">{t("products.description")}</TabsTrigger>
						<TabsTrigger className={'data-[state=active]:text-red-600 data-[state=active]:border-b-red-500 rounded-none bg-transparent uppercase border-b-2 p-4 py-3'} value="images">Images ({product.images.length})</TabsTrigger>
					...
					<TabsContent value="images">
						<div className={'my-7 space-y-5'}>
							{parseProductImages(product.images).map((img, index) => (
								<CloudImage
									key={index} src={img?.url || ""} alt={`product-image-${index}`}
								  width={600} height={500} className={'mx-auto'}
								/>
							))}
						</div>
					</TabsContent>
