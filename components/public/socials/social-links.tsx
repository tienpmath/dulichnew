...
export default function SocialLinks(){
	...
	return (
		<div className={'icon-xh z-10 right-6 bottom-32 fixed hidden lg:block'}>
			<ul className={''}>
				{data.filter(ic => ic.url !=='/').map(icon => (
					<li key={icon.name} className={`${icon.name}`}>
						<Link href={icon.url} target={'_blank'}>
							...
							<span>
								<Image src={icon.icon_path} alt={icon.name} width={40} height={40}/>
							</span>
						</Link>
					</li>
				))}
