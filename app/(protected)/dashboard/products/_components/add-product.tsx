...
import {
	...
	parseProductImages,
	TAddProductSchema,
	TProduct, TProductWithRelation, UpdateProductSchema
...
const AddProduct = (props: {
	...
	const [cateVariants, setCatVariants] = useState(props.categories.find(i => i.id === props.product?.categoryIDs[0])?.variants || props.categories.find(i => i.childrenCategory.map(i => i.id).includes(props.product?.categoryIDs[0]))?.variants || [])
	...
	const formEn = useForm<TAddProductSchema>({
		...
		defaultValues: langEnData.success ? {
			...defaultValueProduct,
			...langEnData.data,
			categoryIDs: langEnData.data.categoryIDs || defaultValueProduct.categoryIDs,
	...
	const [productComments, setProductComments] = useState(props?.product?.comments || [])
											...
											<div className="space-y-4">
												<FormField
													...
													render={({ field: {onChange, value, ...rest} }) => (
														<FormItem className={'flex-col md:flex-row md:gap-5 hidden'}>
												...
												<div className={cn("space-y-4", {"hidden": lang === 'en'})}>
													<FormField
														control={form.control}
														name="price"
														render={({ field }) => (
															...
															<div key={cm.id} className={'flex gap-5'}>
																...
																<div className={'flex-grow justify-end'}>
																	...
																	<AlertDialog>
																		...
																		<AlertDialogContent>
																			<AlertDialogHeader>
																				<AlertDialogTitle>Bạn chắc chắn xóa đánh giá này??</AlertDialogTitle>
																			...
																			<AlertDialogFooter>
																				...
																				<AlertDialogAction onClick={async () => {
																					const {error} = await deleteProductComment(cm.id)
																					...
																					const cmIdx = temp.findIndex(e => e.id === cm.id)
																					temp.splice(cmIdx, 1)
								...
								<DetailPageLayout.Right>
									...
									<div className={'space-y-4 lg:space-y-8 w-full lg:h-[calc(100vh-8rem)] overflow-auto'}>
														...
														<FormItem>
															...
															<Popover>
																<PopoverTrigger asChild>
																	...
																	>
																		<CalendarIcon />
																		{field.value ? format(field.value, "dd LLL, y", {locale: vi}) : <span>Chọn ngày</span>}
																	</Button>
																</PopoverTrigger>
													...
													render={({field}) => (
														<FormItem>
															...
															<Select
																...
																disabled={isPending}
																onValueChange={(v) => {
																	setCatVariants(props.categories.find(i => i.id === v)?.variants || props.categories.find(i => i.childrenCategory.map(i => i.id).includes(v))?.variants || [])
										...
										{(cateVariants.length > 0) && (
											<Card className={cn('p-6 flex flex-col gap-3 w-full', {'hidden': lang === 'en'})}>
												<p>Phân loại</p>
												{cateVariants.filter(v => !Boolean(v.hidden)).map((variant, index) => (
													<Fragment key={index}>
														...
														{Boolean(variant.type ==='options') ? (
															<div className="space-y-0">
																{variant.value.map(c => (
																	<div key={c} className={'space-x-2'}>
																		<Checkbox
																			defaultChecked={Boolean(_.find(form.getValues('variants'), {slug: variant.slug, value: c}))}
																			onCheckedChange={(v) => {
																				const prodVariants = [...form.getValues('variants')]
																				const index = _.findIndex(form.getValues('variants'), {slug: variant.slug, value: c});

																				if(v){
																					prodVariants.push({slug: variant.slug, value: c})
																				} else {
																					prodVariants.splice(index, 1)
																				}
																				form.setValue('variants', prodVariants)
																			}}
																			id={index+slug(c)} value={c}
																		/>
																		<FormLabel className={'text-black'} htmlFor={index+slug(c)}>{c}</FormLabel>
																	</div>
																))}
															</div>
														): (
															<Select
																disabled={isPending}
																defaultValue={_.find(form.getValues('variants'), {slug: variant.slug})?.value || ''}
																onValueChange={(v) => {
																	const prodVariants = [...form.getValues('variants')]
																	const index = _.findIndex(form.getValues('variants'), {slug: variant.slug});
																	if(index >= 0) {
																		prodVariants.splice(index, 1, {slug: variant.slug, value: v})
																	} else {
																		prodVariants.push({slug: variant.slug, value: v})
																	}
																	form.setValue('variants', prodVariants)
																}}
															>
