...
} from "@/actions/products/categories/queries";
...
export async function generateMetadata(
  ...
  const { data, error } = await getProductCategoryBySlug(
  ...
  return {
    title: `${data.name}`,
    description: `Danh sách dịch vụ trong danh mục: ${data.name}`,
    openGraph: {
      title: `${data.name}`,
      description: `Danh sách dịch vụ trong danh mục: ${data.name}`,
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const childCategory =
    params.slug.length > 1
      ? await category.data?.childrenCategory.find(
          (i) => i.slug === params.slug[1]
        )
      : null;
  ...
  if (engVer) {
    category.data.name =
      (category.data.langEn as any)?.name || category.data.name;
    category.data.description = (category.data.langEn as any)?.description;

    if (childCategory) {
      childCategory.name =
        (childCategory.langEn as any)?.name ||
        childCategory.name ||
  ...
  const productsPromise = getProducts({
    ...
    categories:
      childCategory?.id ||
      [category.data.id]
        .concat(category.data.childrenCategory.map((c) => c.id))
        .join("."),
  });
  ...
  const banner = (setting?.value as any[]).find(
    (i) => i.name === "Ảnh banner danh mục"
  );
  const qc_ngang = (setting?.value as any[]).find((i) => i.name === "QC ngang");
  const qc_vuong = (setting?.value as any[]).find(
    (i) => i.name === "Ảnh QC vuông"
  );

  function QcNgang() {
    const videoSetting = (category.data?.settings as TSettingSchema[]).find(
      (i) => i.name === "Youtube Video ID"
    );
  ...
  return (
    <>
      ...
      <div className={"container grid gap-8 grid-cols-3 lg:grid-cols-4"}>
        <div
          className={
            "space-y-6 hidden lg:block sticky top-20 h-[calc(100vh-140px)] overflow-y-auto"
          }
          ...
          <div className={""}>
            ...
            {Boolean(category.data.childrenCategory?.length > 0) && (
              <div className="divide-y divide-zinc-400/30">
                {category.data.childrenCategory
                  .filter((v) => !Boolean(v.hiddenAtSidebar))
                  .sort((a, b) => Number(a.priority) - Number(b.priority))
                  .map((cat, index) => (
                    <Link
                      key={index}
                      className={
                        "block py-3 text-sm font-medium hover:underline uppercase"
                      }
                      href={`/${category.data?.slug}/${cat.slug}`}
                    >
                      {cat.name}
                    </Link>
                  ))}
          ...
          <div>
            {qc_vuong.data.map((img, index) => (
              <Link
                ...
                className={
                  !String(qc_vuong.data_links[index])
                    ? "pointer-events-none"
                    : ""
                }
        ...
        <div className="col-span-3 min-h-screen">
            ...
            <div className="flex items-start py-2 justify-between">
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button
                    variant={"ghost"}
                    className={"lg:hidden text-center uppercase block"}
                  >
                  ...
                  <ScrollArea className={"flex-grow"}>
                    <div className={"space-y-6"}>
                      <div className={""}>
                        ...
                        {Boolean(
                          category.data.childrenCategory?.length > 0
                        ) && (
                          <div className="divide-y divide-zinc-400/30">
                            {category.data.childrenCategory
                              .filter((v) => !Boolean(v.hiddenAtSidebar))
          ...
          <React.Suspense
            fallback={
              <p className="text-center mx-auto my-8 text-xl">Loading</p>
            }
          >
