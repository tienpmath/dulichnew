...
export default function ProductFeatures(props: {
	...
	data = data.map((i, index) => ({
		...i,
		img: (props.data?.value as any[]).find(i => i.name === 'Sản phẩm').data[index] || ""
	}))
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 justify-items-center">
					{data.map(d => (
						<div key={d.title} className={'w-full'}>
							<Link href={d.path} className="group block aspect-square overflow-hidden w-full">
								<CloudImage
									src={d.img}
							...
							<div className={'border-2 px-2 py-2 flex flex-col lg:flex-row gap-3 items-center justify-between w-full'}>
